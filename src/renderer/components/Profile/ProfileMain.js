...
import DeleteAll from '../PopUp/DeleteAll';
import ProfileModal from '../PopUp/ProfileModal';

export const ProfileMain = () => {
  const { theme } = useContext(ThemeContext);
  const { profileGroups, setProfileGroups, activeProfileGr, setActiveProfileGr, activeProfileGrg, setActiveProfileGrg, mode, setMode } = useGlobal();
  const defaultGroup = {
    name: 'Profile Group 1',
    id: profileGroups.length + 1,
    profilesArr: [],
  };
  ...
  /*group*/
  console.log(setActiveProfileGrg);
  console.log(activeProfileGrg);
  return (
    <div>
        ...
        <div className={`${theme}__ProfileHeader`}>
          <div className={`${theme}__ProfileGroup`}>
            <div className="__ProfileGroup__title">
              <span className={`${theme}__groupTitle`}>Profile Groups</span>
              <span className={`${theme}__subTitle`}>{profileGroups.length} Groups</span>
            </div>
          ...
          <div className="ProfileContent__header">
            <div className="Profile_header_left">
              <div className="profile_header_title">
                <span className={`${theme}__groupTitle`}>Profiles</span>
                <span className={`${theme}__subTitle`}>{activeProfileGr?.profilesArr.length} Profiles</span>
              </div>
              ...
              {profileGroups &&
                ...
                profileGroups.map((Profile, index) => (
                  <ProfileAccountGroup
                    ...
                    idx={index}
                    active={Profile.id === activeProfileGr.id}
          ...
          <div className={`${theme}_Profiles`}>
            <div className="Profile__content">
              <div className="Profile__content__body">
                {activeProfileGr && activeProfileGr?.profilesArr.map((item, i) => <ProfileItem key={i} Profile={item} active={item.id === activeProfileGrg.id} setActive={setActiveProfileGrg} />)}
              </div>
            </div>
          </div>
        </div>
      </div>
      {groupModal && <GroupModal closeModal={closeModal} addGroup={addGroup} title={'Profile'} />}
      {openModal && <ProfileModal closeModal={closeAddModal} />}
